...
class Experiment:
    ...
    def mismatched_negativity(self):
        ...
        movie_stimuli = pd.read_csv(self.__path + '/mismatched_negativity_task/MMN_movie_stimuli.csv')
        ...
        for i in range(1, 7):
            movie_stim = movie_stimuli.loc[movie_stimuli['block'] == i, ['stimulus']]
            movie_stim = MovieStim(movie_stim, autoStart=True)
            auditory_stim = auditory_stimuli.loc[auditory_stimuli['block'] == i]

            if movie_stim == NOT_STARTED:
                movie_stim.play()
            # self.__win.callonFlip(self.__port.write, ''.encode())
            for k in auditory_stim:
                self.__clock.reset()
                trigger_sent = False
                trig = auditory_stim.loc[:, 'trigger']
                while self.__clock.getTime() < duration:
                    if not trigger_sent:
                        # self.__win(self.__port.write, trig.encode())
                        trigger_sent = True

                    # if auditory_stim

                df = pd.DataFrame({'condition': x, 'sound': y})
                MMN_data.append(df)
        ...
        # TODO: reinforced practice trials, data saving, randomisation, event triggers

    def visual_stimulation(self):
        '''
        Task 4: visual stimulation paradigm with breath holding.
    ...
    def naturalistic_motor_task(self):
        ...
        for j in range(len(naturalistic_motor_stims)):
            ...
            # if naturalistic_motor_audio.status == FINISHED:
            #     self.__win.flip()
...
e = Experiment(portname=None, fullscreen=True)
e.test()
